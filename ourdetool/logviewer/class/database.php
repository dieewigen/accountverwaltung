...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
		// Use global database connection instead of creating a new one
		if (isset($GLOBALS['dbi']) && $GLOBALS['dbi']) {
			$this->_resource = $GLOBALS['dbi'];
		} else {
			// Fallback: create new connection if global one doesn't exist
